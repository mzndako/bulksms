				...
				<div class="col-md-12">
					...
					<div class="row myuserform collapse <?=!empty($mysearch) && !empty($ms->get("lastlogin_date1"))?"in":"";?>">
						<div class="col-md-3 form-group">
							<div class="checkbox">
								<label>
									<input onchange="show_div(this, '.lastlogin')" type="checkbox" name="activate_lastlogin" <?=p_checked(!empty($ms->get("lastlogin_date1")));?>  value="1"/> Last Login
								</label>
							</div>
						</div>
...
//					$title = "<div align='left'>Name: $row[fname] $row[surname]<br>Sex: $row[sex]<br>Balance: ".format_amount($row['balance'])."<br>Email: $row[email]</div>";
						...
						if(!$is_mobile) {
							?>
							<?= convert_to_datetime($row['last_login']); ?>
							<?php
						}else {
							?>
							<time
